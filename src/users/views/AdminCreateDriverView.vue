...
import { createDriverSchema } from '../schemas/createDriverSchema';
...
const validateField = async (fieldName) => {
  try {
    await createDriverSchema.validateAt(fieldName, form.value)
...
const validateForm = async () => {
  ...
  try {
    await createDriverSchema.validate(form.value, { abortEarly: false })
    return true
  } catch (error) {
...
const submitForm = async () => {
  ...
  try {
    ...
    successMessage.value = 'Driver creado exitosamente'
...
<template>
  <div class="container mx-auto p-6 max-w-2xl">
    <h1 class="text-3xl font-bold mb-6">Crear Driver</h1>
    ...
    <form @submit.prevent="submitForm" class="space-y-6">
      ...
      <div>
        <label class="block text-sm font-medium mb-2">Vehículo a utilizar</label>
        <input
          v-model="form.vehicle"
          type="text"
          placeholder="Ej: Moto, Auto, Bicicleta"
          class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          @blur="validateField('vehicle')"
        />
